...
//! [`FetchOutcome::BudgetExhausted`] or [`ContextLookupOutcome::BudgetExhausted`]. Between turns,
...
//! | enabled tool-family turn budget | requests per attempt | 2 plus each enabled family |
...
///
/// ## 4 -> 5: operator-provided prompt context
///
/// Domain vocabulary and classification guidance can turn a previously undetermined answer into
/// a placement, so queued proposals must be asked once with the configured context available.
pub const CLASSIFIER_GENERATION: u32 = 5;
