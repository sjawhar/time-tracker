...
fn bounded_text(bytes: Vec<u8>, stream: &str) -> Result<String, ContextProviderError> {
    ...
    if text.len() <= MAX_CONTEXT_LOOKUP_RESULT_BYTES {
        return Ok(text);
    }

    let mut end = MAX_CONTEXT_LOOKUP_RESULT_BYTES - TRUNCATION_MARKER.len();
...
mod tests {
    ...
    fn input() -> ClassificationInput {
        ClassificationInput {
            ...
            machine: None,
            cwd: None,
            starting_prompt: Some("Classify Apollo work".to_owned()),
    ...
    fn a_failed_lookup_command_is_a_clean_tool_result_and_the_classification_continues() {
        ...
        // When
        let output = classifier.classify(&input(), &[]).unwrap();
    ...
    #[cfg(unix)]
    #[test]
    fn a_lookup_command_timeout_is_a_clean_tool_error() {
        // Given: a command that never reaches its result. `exec` makes the fixture itself
        // the sleeping process, so the timeout kills every writer holding its pipes. The
        // budget is injected short so the test proves the kill without paying the
        // production ten seconds — the loaded-host flake this replaced blew a wall-clock
        // assertion while a real 10s sleep ran under a full parallel suite.
        let temp = TempDir::new().unwrap();
        let command = lookup_command(&temp, "exec sleep 60");
        let provider =
            CommandContextProvider::with_timeout(&command, Duration::from_millis(200)).unwrap();

        // When
        let started = Instant::now();
        let error = provider.lookup("Apollo").unwrap_err();

        // Then: no classification-facing caller can hang forever or receive a process
        // error. The generous outer bound tolerates loaded test hosts while proving the
        // 60-second child was stopped by the configured limit.
        assert!(
            error.to_string().contains("timed out after 0.2 seconds"),
            "{error}"
        );
    ...
    fn lookup_results_are_truncated_with_a_marker() {
        ...
        // When
        let text = provider.lookup("Apollo").unwrap();
    ...
    fn configured_lookup_command_appends_the_query_as_one_argument() {
        ...
        // When
        let text = provider.lookup("Apollo migration project").unwrap();
